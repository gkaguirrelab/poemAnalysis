function [PA,EIGENS,MCORR]=polychoric_CPGadapted(MSCORES,missingcode,replicates,smoothing,centered)     
...
% Carlyn Patterson Gentile adaptations: 
% - To get correlation matrix, MCORR was added as a 3rd output value. 
% - 'centered' was added as a variable to specify if the mean will be
% subtracted from the correlation matrix. 0=false, 1=true
...
if centered==1 % subtract nondiagonal mean
    Temp=MCORRPOLY;
    Temp(Temp==1)=NaN;
    nondiagM=mean(mean(Temp(~isnan(Temp))));
    MCORRPOLY=MCORRPOLY-nondiagM;
else
end
